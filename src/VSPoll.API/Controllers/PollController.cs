...
public class PollController : Controller
{
    ...
    /// <summary>
    /// Gets the vote(s) of the specified user
    /// </summary>
    /// <param name="id">The poll id</param>
...
#pragma warning restore IDE0079
    public async Task<ActionResult> Vote(Guid id, [FromBody] RankedVote vote)
    {
        ...
        if (vote.Options is null || !vote.Options.Any())
            return BadRequest(poll.VotingSystem switch
            {
                VotingSystem.SingleOption => "An option should be inputted",
                _ => "At least one option should be inputted",
            });
